...
describe("test/unit/saveMediaToDb/failure/saveMediaToDB_fail.js", function () {
 
  context("[evolving eel] should", function () {

    testHelpers.dropMediaCollectionDoneCallback(the_media);
    ...
    it("fail when header does not match", function (done) {
      ...
      save_rss_items.saveItemsToDb(variables_tsv, parser_tsv, the_media, media_constants.TEST_DATA, verify_tsv)
        .catch(function (e) {
          expect(e.message).toBe("Invalid Record Length: expect 12, got 13 on line 2" );
          done();
        });
    });
  });
