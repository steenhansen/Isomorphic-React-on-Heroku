...
describe('test/integration/saveTestToDb/success/saveTestToDb.js', function () {
    ...
    before(function (done) {testHelpers.unlink_async(produced_file_1, done)})
        before(function (done) {testHelpers.unlink_async(produced_file_2, done)})
    ...
    context("[dread deer] good data should", function () {
        ...
        before(function (done) {
            ...
            screenOutput.html_saveTestToDb_P1(variables_tsv, parser_tsv, rsd_media, the_information, media_file_loc)
                .then(function () {
                    return screenOutput.html_saveTestToRss_P2(variables_tsv, parser_tsv, rsd_media, media_file_loc)
                })
                .then(function (html) {
        ...
        it('produce correct html', function () {
            var the_diff = testHelpers.whiteSpaceFileDiff(produced_file_1, expected_rss)
    ...
    context("[eloping elephants] good data should", function () {
        ...
        before(function (done) {
            rsd_media.getDocument('rss_document', 'all_rss_xml', media_constants.TEST_DATA)
        ...
        it('[aardvarks anonymous] produce correct rss xml', function () {
            var the_diff = testHelpers.whiteSpaceFileDiff(produced_file_2, expected_rss)
