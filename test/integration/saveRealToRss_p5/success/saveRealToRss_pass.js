...
describe('test/integration/saveRealToRss_p5/success/saveRealToDb.js', function () {
    ...
    before(function (done) {testHelpers.unlink_async(produced_p5_html, done)})
    before(function (done) {testHelpers.unlink_async(produced_p5_db, done)})
    ...
    context("[catapulting cars] good data should", function () {
        before(function (done) {
            ...
            screenOutput.html_saveRealToDb_P4(variables_tsv, parser_tsv, rsd_media, the_information, media_file_loc)
             .then( function(){
                 return screenOutput.html_saveRealToRss_P5(variables_tsv, parser_tsv, rsd_media, media_file_loc)
             })
             .then(function (p5_html) {
                 return miscMethods.saveLocalFile(produced_p5_html, p5_html)
             })
                .then( function(){
                    return rsd_media.getDocument('rss_document', 'all_rss_xml', media_constants.REAL_DATA)
                })
        ...
        it('produce correct p5 html', function () {
            var the_diff = testHelpers.whiteSpaceFileDiff(produced_p5_html, expected_p5_html)
            expect(the_diff).toEqual('', 'expected p5 html !== actual p5 html')
        })

        it('produce correct p5 data', function () {
            var the_diff = testHelpers.whiteSpaceFileDiff(produced_p5_db, expected_p5_db)
