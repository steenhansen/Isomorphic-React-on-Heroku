...
<script   src="${vendors_js}"></script> 
  ...
  marshallServerHtml: function (media_type, props_array, desk_mobile_template, host_url, media_information, media_options = {}) {
    ...
    var media_description = miscMethods.getFillSwig(expected_file, {
      site_url: site_url
    });
    var props = {
      data_list: props_array,
      /// different name, real_items
    ...
    var content_div = React.createElement('div', {
      id: div_name,
    ...
    if (media_type === 'rsd') {
      var media_props_script = React.createElement('script', {
        dangerouslySetInnerHTML: {
          __html: 'var RSD_MEDIA_PROPS_SCRIPT = ' + miscMethods.safeStringify(props)
        }
      });
    } else if (media_type === 'pdf') {
      var media_props_script = React.createElement('script', {
        dangerouslySetInnerHTML: {
          __html: 'var PDF_MEDIA_PROPS_SCRIPT = ' + miscMethods.safeStringify(props)
        }
      });
    } else if (media_type === 'podcast') {
      var media_props_script = React.createElement('script', {
        dangerouslySetInnerHTML: {
          __html: 'var PODCAST_MEDIA_PROPS_SCRIPT = ' + miscMethods.safeStringify(props)
        }
      });
    } else {
