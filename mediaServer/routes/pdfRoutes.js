...
var variables_tsv = di_factory.VariablesTsvUrlCreate(
  pdf_information.google_variables_tab
);
var parser_tsv = di_factory.ParserTsvUrlCreate(
  pdf_information.google_media_tab
);
...
module.exports = function (app) {
  ...
  app.get(
    media_url_dirs.URL_HTML_ADMIN_checkItemsForCron_P0,
    basic_http_auth,
    function (req, res) {
      if (parser_tsv instanceof Error) {
        var check_for_cron_html = media_file_loc.htmlFiles(
          'checkItemsForCron_fail'
        );
  ...
  app.get(media_url_dirs.URL_HTML_PUBLIC_TABLE, function (req, res) {
    ...
    var promise_pdfs_items = pdf_media.currentList(
      react_constants.MAX_DESKTOP_ITEMS
    );
  ...
  app.get(media_url_dirs.URL_HTML_MOBILE_TABLE, function (req, res) {
    var pdf_media = createMedia();
    var promise_pdfs_items = pdf_media.currentList(
      react_constants.MAX_MOBILE_ITEMS
    );
    var promise_pdfs_count = pdf_media.countPdfs();
    Promise.all([promise_pdfs_items, promise_pdfs_count])
      .spread(function (pdf_items, pdfs_count) {
