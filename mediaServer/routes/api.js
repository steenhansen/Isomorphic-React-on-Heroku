...
var rsd_di_factory = rootAppRequire('mediaServer/modules/base/diFactory')(
  rsd_information
);

var pdf_information = rootAppRequire('mediaServer/modules/pdfSchema');
var pdf_di_factory = rootAppRequire('mediaServer/modules/base/diFactory')(
  pdf_information
);

var podcast_information = rootAppRequire('mediaServer/modules/podcastSchema');
var podcast_di_factory = rootAppRequire('mediaServer/modules/base/diFactory')(
  podcast_information
);
...
function logExpressErrors (e, req, res, next) {
  global.Method_logger.chronicle(
    'error',
    'express-error',
    module.filename,
    ' e.stack',
...
module.exports = function (public_static_files, localhost_port) {
  ...
  app.get('/all', function (req, res) {
    ...
    var promise_pdf_media = pdf_media.currentList(
      react_constants.MAX_DESKTOP_ITEMS
    );
    var promise_pdfs_count = pdf_media.countPdfs();
    var promise_podcast_media = podcast_media.currentList(
      react_constants.MAX_DESKTOP_ITEMS
    );
    var promise_rsd_media = rsd_media.currentList(
      react_constants.MAX_DESKTOP_ITEMS
    );

    Promise.all([
      promise_pdf_media,
      promise_podcast_media,
      promise_rsd_media,
      promise_pdfs_count
    ])
      .spread(function (pdf_props, podcast_props, rsd_props, pdfs_count) {
        let media_options = {
          pdfs_count: pdfs_count / 2
        };
        var pdf_html = send_table.marshallServerHtml(
          'pdf',
          pdf_props,
        ...
        var podcast_html = send_table.marshallServerHtml(
          'podcast',
          podcast_props,
          media_url_dirs.PUBLIC_SHOW_EXPLAIN,
          req.headers.host,
        ...
        var rsd_html = send_table.marshallServerHtml(
          'rsd',
          rsd_props,
          media_url_dirs.PUBLIC_SHOW_EXPLAIN,
          req.headers.host,
        ...
        var page_html = send_table.encaseAll(
          rsd_html,
          podcast_html,
          pdf_html,
          req.headers.host
