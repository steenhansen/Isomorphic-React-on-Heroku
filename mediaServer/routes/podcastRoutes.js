...
var di_factory = rootAppRequire('mediaServer/modules/base/diFactory')(podcast_information)
var save_rss_items = rootAppRequire('mediaServer/modules/base/saveRssItems')(di_factory)
var variables_tsv = di_factory.VariablesTsvUrlCreate(podcast_information.google_variables_tab)
var parser_tsv = di_factory.ParserTsvUrlCreate(podcast_information.google_media_tab)
var basic_http_auth = miscMethods.basicHttpAuth
var media_file_loc = rootAppRequire('mediaServer/modules/base/mediaFileLoc')(podcast_information)   
...
function createMedia(){
    ...
    app.get(media_url_dirs.URL_HTML_ADMIN_saveTestToDb_P1, basic_http_auth, function (req, res) {
        if (parser_tsv instanceof Error) {
          ...
          var the_media = createMedia()
            ...
            screenOutput.html_saveTestToDb_P1(variables_tsv, parser_tsv, the_media, podcast_information, media_file_loc)
    ...
    app.get(media_url_dirs.URL_HTML_ADMIN_viewTestRss_P3, basic_http_auth, function (req, res) {
        var page_html = screenOutput.html_viewTestRss_P3(podcast_information, media_file_loc)
        res.send(page_html)
    })
    ...
    app.get(media_url_dirs.URL_HTML_ADMIN_saveRealToDb_P4, basic_http_auth, function (req, res) {
     //   var the_media = di_factory.PodcastMediaCreate()
       var the_media = createMedia()
        screenOutput.html_saveRealToDb_P4(variables_tsv, parser_tsv, the_media, podcast_information, media_file_loc)
            .then(function (page_html) {
       ...
       // var the_media = di_factory.PodcastMediaCreate()
        ...
        the_media.getDocument('rss_document', 'all_rss_xml', media_constants.REAL_DATA)
            .then(function (podcast_rss_xml) {
                ...
                var current_rss = miscMethods.ignoreFutureItems(podcast_rss_xml, now_yyyy_mm_dd_hh_mm)
                if (!current_rss) {
    ...
    app.get(media_url_dirs.URL_HTML_PUBLIC_SHOW_PLAYER, function (req, res) {
      ...
      //  var the_media = di_factory.PodcastMediaCreate()
        var the_media = createMedia()
        screenOutput.htmlPublicShowPlayer(id, the_media, podcast_information, media_file_loc)
    ...
    app.get(media_url_dirs.URL_HTML_ADMIN_SHOW_ALL_OUTPUT, basic_http_auth, function (req, res) {
        var host_url = 'http://' + req.headers.host
        var page_html = screenOutput.htmlAdminShowAll(podcast_information, media_file_loc, host_url)
        res.send(page_html)
    })
