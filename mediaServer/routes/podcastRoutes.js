...
var variables_tsv = di_factory.VariablesTsvUrlCreate(
  podcast_information.google_variables_tab
);
var parser_tsv = di_factory.ParserTsvUrlCreate(
  podcast_information.google_media_tab
);
...
module.exports = function (app) {
  function createMedia () {
    var podcast_media = di_factory.PodcastMediaCreate(); // //////////////// one instance
    return podcast_media;
  }

  app.get(
    media_url_dirs.URL_HTML_ADMIN_saveTestToDb_P1,
    basic_http_auth,
    function (req, res) {
  ...
  app.get(
    media_url_dirs.URL_HTML_ADMIN_saveTestToRss_P2,
    basic_http_auth,
    function (req, res) {
      // var the_media = di_factory.PodcastMediaCreate()
  ...
  app.get(
    media_url_dirs.URL_HTML_ADMIN_viewTestRss_P3,
    basic_http_auth,
    function (req, res) {
      var page_html = screenOutput.html_viewTestRss_P3(
        podcast_information,
        media_file_loc
      );
      res.send(page_html);
  ...
  app.get(
    media_url_dirs.URL_XML_ADMIN_IFRAME_showTestRss_P3,
    basic_http_auth,
    function (req, res) {
      //   var the_media = di_factory.PodcastMediaCreate()
      ...
      the_media
        ...
        .then(function (podcast_rss_xml) {
          // throw new Error ('exception test - URL_XML_ADMIN_IFRAME_showTestRss_P3')    //  @@@@
          res.header('Content-Type', 'application/rss+xml');
          res.send(podcast_rss_xml);
        })
        .catch(function (e) {
          var html_error = global.Method_logger.exception(
            'URL_XML_ADMIN_IFRAME_showTestRss_P3',
            module.filename,
            e
          );
          res.send(html_error);
  ...
  app.get(
    media_url_dirs.URL_HTML_ADMIN_saveRealToDb_P4,
    basic_http_auth,
    function (req, res) {
      //   var the_media = di_factory.PodcastMediaCreate()
      var the_media = createMedia();
      screenOutput
        .html_saveRealToDb_P4(
          variables_tsv,
          parser_tsv,
          the_media,
          podcast_information,
        ...
        .catch(function (e) {
          var html_error = global.Method_logger.exception(
            'URL_HTML_ADMIN_saveRealToDb_P4',
            module.filename,
            e
          );
          res.send(html_error);
  ...
  app.get(media_url_dirs.URL_XML_ADMIN_IFRAME_rss_P5, function (req, res) {
    ...
    var the_media = createMedia();
    the_media
      .getDocument('rss_document', 'all_rss_xml', media_constants.REAL_DATA)
      .then(function (podcast_rss_xml) {
        // throw new Error ('exception test - URL_XML_ADMIN_IFRAME_rss_P5')

        var now_yyyy_mm_dd_hh_mm = miscMethods.serverYYYYMMDDHHmm();
        var current_rss = miscMethods.ignoreFutureItems(
          podcast_rss_xml,
          now_yyyy_mm_dd_hh_mm
        );
        if (!current_rss) {
          current_rss = podcast_rss_xml;
        }
        miscMethods.serveGzipContent(
          req,
          res,
          'application/rss+xml',
          current_rss
  ...
  app.get(
    media_url_dirs.URL_HTML_ADMIN_saveRealToRss_P5,
    basic_http_auth,
    function (req, res) {
      //   var the_media = di_factory.PodcastMediaCreate()
      var the_media = createMedia();
      ...
      screenOutput
        ...
        .catch(function (e) {
          var html_error = global.Method_logger.exception(
            'URL_HTML_ADMIN_saveRealToRss_P5',
            module.filename,
            e
          );
          res.send(html_error);
  ...
  app.get(media_url_dirs.URL_HTML_PUBLIC_SHOW_PLAYER, function (req, res) {
    ...
    var the_media = createMedia();
    screenOutput
      .htmlPublicShowPlayer(id, the_media, podcast_information, media_file_loc)
  ...
  app.get(
    media_url_dirs.URL_HTML_ADMIN_SHOW_ALL_OUTPUT,
    basic_http_auth,
    function (req, res) {
      var host_url = 'http://' + req.headers.host;
      var page_html = screenOutput.htmlAdminShowAll(
        podcast_information,
        media_file_loc,
        host_url
      );
  ...
  app.get(media_url_dirs.URL_HTML_PUBLIC_TABLE, function (req, res) {
    ...
    var the_media = createMedia();
    the_media
      .currentList(react_constants.MAX_DESKTOP_ITEMS)
      .then(function (props_array) {
        // throw new Error ('exception test - URL_HTML_PUBLIC_TABLE')
        var table_html = send_table.marshallServerHtml(
          'podcast',
          props_array,
          media_url_dirs.PUBLIC_SHOW_EXPLAIN,
          req.headers.host,
          podcast_information
        );
        var page_html = send_table.encaseTable(
          'podcast_3rd_bundle.js',
          table_html,
          req.headers.host,
          'desktop_device',
          'The SFFaudio Podcast - SFFaudio'
        );
        miscMethods.serveGzipContent(
          req,
          res,
  ...
  app.get(media_url_dirs.URL_HTML_MOBILE_TABLE, function (req, res) {
    ...
    var the_media = createMedia();
    the_media
      .currentList(react_constants.MAX_MOBILE_ITEMS)
      .then(function (props_array) {
        //  throw new Error ('exception test - URL_HTML_MOBILE_TABLE')
        var table_html = send_table.marshallServerHtml(
          'podcast',
          props_array,
          media_url_dirs.MOBILE_EXPLAIN,
          req.headers.host,
          podcast_information
        );
        var page_html = send_table.encaseTable(
