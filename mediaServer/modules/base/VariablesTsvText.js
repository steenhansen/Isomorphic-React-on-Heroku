...
VariablesTsvText.prototype.deriveAll = function (
  the_rows,
  current_media,
  tsv_variables,
  offset_minutes
  ...
  var multiple_rows = current_media.splitVersions(
    the_rows,
    offset_minutes,
    tsv_variables
  );
    ...
    .then(function (variable_rows) {
      ...
      for (var value_key in variable_rows) {
        if (variable_rows.hasOwnProperty(value_key)) {
          var variable_name = miscMethods.trimString(
            variable_rows[value_key]["variables"]
          );
          var variable_value = miscMethods.trimString(
            variable_rows[value_key]["values"]
          );
