...
var miscMethods = {
  ...
  readLocalFile: function (file_pathname) {
    return fsAsync
      .readFileAsync(file_pathname, "utf8")
  ...
  ignoreFutureItems: function (rsd_rss_xml, now_yyyy_mm_dd_hh_mm) {
    ...
    for (var i = 1; i < number_entries; i++) {
      ...
      if (rsd_date > now_yyyy_mm_dd_hh_mm) {
        ...
        /// if first item viewable, then all are
        return false;
      }
    }
  ...
  connectToMongoose: function (mongoose_connect_name, port_number) {
    if (!mongoose.connection.db) {
      mongoose.connect(mongoose_connect_name, {useNewUrlParser: true, useUnifiedTopology: true},
        function (e) {
          if (e) {
            console.log('mongoose connect to atlas error', e);
            throw e;
          }
        }
      );
  ...
  saveLocalFile: function (file_pathname, file_data) {
    return fsAsync.writeFileAsync(file_pathname, file_data).catch(function (e) {
      throw e;
    });
  },
  ...
  basicHttpAuth: function (req, res, next) {
    ...
    if (
      user.name === process.env.HTACCESS_USERNAME &&
      user.pass === process.env.HTACCESS_PASSWORD
    ) {
      return next();
    } else {
      return unauthorized(res);
  ...
  convertMediaPublishDate: function (date_hour) {
    var publish_date_hour = moment(date_hour, [
      "YYYY-MM-DD H:mm",
      "YYYY-M-DD H:mm",
      "YYYY-MM-D H:mm",
      "YYYY-M-D H:mm",
