...
/// we should log this
  try {
   ...
   return the_collection.drop();  
  } catch (error) {
       console.warn('the_collection.drop() collection may not exist', the_collection);             //this may be fine
  }
...
BaseMedia.prototype.deriveDate = function (data_columns, offset_minutes) {
  ...
  var media_publish_long = miscMethods.convertMediaPublishDate(
    media_publish_date
  );
...
BaseMedia.prototype.splitVersions = function (
  media_rows /*:Array<MediaRows>*/
) {
  return media_rows;
};
...
BaseMedia.prototype.rssFeed = function (
  data_rows,
  tsv_variable_information,
  item_template,
  page_template_file,
  itunes_summary,
  rss_episodes_range,
  ...
  var template_vars = this._pageTemplateVars(
    rss_items_html,
    tsv_variable_information,
    itunes_summary,
    host_url
...
BaseMedia.prototype.collectionAsString_test = function (itemsDb) {
  var collection_promise = itemsDb.collection
    .find({})
    .sort({ "episode number": -1 })
    .toArray();
  return collection_promise.then(function (collection_arr) {
...
BaseMedia.prototype.getDocument = function (
  itemsDb,
  document_id /*:string*/,
  field_id /*:string*/,
  real_or_test /*:string*/
...
//console.log('thanks obama 9999999999999999999999999999999999999999999999999999999999');
//console.log('media_constants.TEST_DATA', media_constants.TEST_DATA)
//console.log('document_id', document_id)
  ...
  //console.log('++++++++++++++++++++++++++++++++++++++++++++++++++', the_document_id)
  
  return itemsDb.findOneAsync({ _id: the_document_id }).then(function (data) {
    if (data === null) {
      let mongo_error =
        "document_id==" + the_document_id +
        "~~~~~" +
        "field_id==" + field_id +
        "~~~~~" +
        "real_or_test==" + real_or_test +
        "~~~~~" +
        " does not exist in Mongo db";
...
BaseMedia.prototype.deleteItems = function (itemsDb, real_or_test /*:string*/) {
  ...
  //console.log('BaseMedia.prototype.deleteItems' , realOrTest_indexKeyExists)  

  return itemsDb.deleteMany(realOrTest_indexKeyExists);

//  return itemsDb.removeAsync(realOrTest_indexKeyExists);
};

BaseMedia.prototype.deleteDocument = function (
  general_db,
 ...
 //var str = JSON.stringify(general_db);
//console.log('general_db' , str)   deleteMany


return general_db.deleteMany({
  _id: document_name_remove,
  real_or_test: real_or_test,
});


  // return general_db.removeAsync({
  //   _id: document_name_remove,
  //   real_or_test: real_or_test,
  // }); // general_db
};

BaseMedia.prototype.saveDocument = function (
  document_name,
  variable_name,
  the_value,
  real_or_test
  ...
  if (media_constants.TEST_DATA === real_or_test) {
    var description_columns = {
      _id: this._test_id_prefix + document_name,
      real_or_test: real_or_test,
    };
  } else {
    var description_columns = {
...
BaseMedia.prototype.saveItem = function (
  data_columns,
  real_or_test /*:string*/,
  derived_columns
) {
...
BaseMedia.prototype.upsertDocument = function (
  itemsDb,
  document_name,
  variable_name,
  the_value,
...
BaseMedia.prototype._pageTemplateVars = function (
  media_items_html,
  tsv_variable_information,
  itunes_summary,
  host_url
...
BaseMedia.prototype.currentList = function (max_records, media_items_db) {
  ...
  if (max_records > 0) {
    var collection_promise = media_items_db.collection
      .find(valid_episodes)
      .sort({ "episode number": -1 })
      .limit(max_records)
      .toArray();
