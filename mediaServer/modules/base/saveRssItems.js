...
module.exports = function (_di_factory) {
  ...
  var save_rss_items = {
    saveRssToDb: function (
      ...
      var promise_tsv_variables = variables_tsv.allVariables(
        verify_tsv_variables
      );
      ...
      return Promise.all([promise_tsv_variables, promise_the_rows]).spread(
        function (tsv_variables, the_rows) {
          //throw new Error ('exception test - saveRssToDb - 2')
          var offset_minutes =
            60 * tsv_variables.hours_offset +
            1 * tsv_variables.post_early_min_rss;
          var derived_rows = variables_tsv.deriveAll(
            the_rows,
            the_media,
            tsv_variables,
            offset_minutes
          ...
          return the_media
            .getDocument("description", "itunes_summary", real_or_test)
            .then(function (itunes_summary) {
              //throw new Error('exception test - saveRssToDb - 3')
              var rss_xml = the_media.rssFeed(
                derived_rows,
                tsv_variables,
                item_template,
                feed_template,
      ...
      return Promise.all([promise_tsv_variables, promise_the_rows]).spread(
        function (tsv_variables, the_rows) {
          ...
          if (test_values_regex !== "") {
            ...
            var itunes_description_em_dash = miscMethods.replace2withMdash(
              variables_tsv._captured_variables.itunes_description
            );
            ...
            return version_storage
              .saveNewVersion(derived_rows, real_or_test)
              .then(function (number_rows) {
             //   console.log(' qbert num rows', number_rows)
    ...
    checkItemsForCron: function (variables_tsv, parser_tsv, the_media) {
      // q*bert
      var verify_tsv = di_factory.VerifyTsvDataRowsCreate();
      var verify_tsv_variables = di_factory.VerifyTsvVariablesCreate();
      var promise_tsv_variables = variables_tsv.allVariables(
      ...
      return Promise.all([promise_tsv_variables, promise_the_rows]).spread(
        function (tsv_variables, the_rows) {
          ...
          if (test_values_regex !== "") {
            ...
            var first_pdf = save_rss_items.bookTitleAuthor(
              the_rows,
              the_rows.length - 1
            );
            var second_pdf = save_rss_items.bookTitleAuthor(
              the_rows,
              the_rows.length - 2
            );
    ...
    checkEpisodeNumbers: function (the_rows) {
      ...
      if (biggest_episode !== the_rows.length) {
        var error_mess =
          "Episode numbers do not match. Last episode #" +
          biggest_episode +
          " <> rows" +
          the_rows.length;
