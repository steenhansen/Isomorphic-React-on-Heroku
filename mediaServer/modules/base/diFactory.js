...
module.exports = function (media_information) {
  var diFactory = {
    ParserTsvTextCreate: function (tsv_text) {
      ...
      if (missing_methods) {
        throw new Error(
          "ParserTsvTextCreate.missing_methods() " + missing_methods
        );
      } else {
        var parser_tsv_text = new ParserTsvText(
          csv_parse,
          tsv_text,
          media_constants.CSV_PARSER_OPTIONS
        );
    ...
    ParserTsvFileCreate: function (tsv_pathname) {
      ...
      if (missing_methods) {
        throw new Error(
          "ParserTsvFileCreate.missing_methods() " + missing_methods
        );
      } else {
        var parser_tsv_file = new ParserTsvFile(
          csv_parse,
          tsv_pathname,
          media_constants.CSV_PARSER_OPTIONS
        );
    ...
    ParserTsvUrlCreate: function (tsv_url) {
      ...
      if (missing_methods) {
        throw new Error(
          "ParserTsvUrlCreate.missing_methods() " + missing_methods
        );
      } else {
        ...
        if (validUrl.isUri(tsv_url)) {
          try {
            ...
            var parser_tsv_url = new ParserTsvUrl(
              csv_parse,
              tsv_url,
              media_constants.CSV_PARSER_OPTIONS
            );
    ...
    VariablesTsvTextCreate: function (tsv_text) {
      ...
      if (missing_methods) {
        throw new Error(
          "VariablesTsvTextCreate.missing_methods() " + missing_methods
        );
      } else {
        var variables_tsv_text = new VariablesTsvText(
          csv_parse,
          tsv_text,
          media_constants.CSV_PARSER_OPTIONS
        );
    ...
    VariablesTsvFileCreate: function (tsv_pathname) {
      ...
      if (missing_methods) {
        throw new Error(
          "VariablesTsvFileCreate.missing_methods() " + missing_methods
        );
      } else {
        var variables_tsv_file = new VariablesTsvFile(
          csv_parse,
          tsv_pathname,
          media_constants.CSV_PARSER_OPTIONS
        );
    ...
    VariablesTsvUrlCreate: function (tsv_url) {
      ...
      if (validUrl.isUri(tsv_url)) {
        var variables_tsv_url = new VariablesTsvUrl(
          csv_parse,
          tsv_url,
          media_constants.CSV_PARSER_OPTIONS
        );
    ...
    VerifyTsvVariablesCreate: function () {
      ...
      if (
        heading_vars instanceof Array &&
        must_contain instanceof Object &&
        contain_errors instanceof Object &&
        tsv_variables instanceof Array
      ) {
        var verify_vars_tsv = new VerifyTsv(
          heading_vars,
          must_contain,
          contain_errors,
          tsv_variables
    ...
    PdfMediaCreate: function () {
      ...
      var pdf_storage = new PdfMedia(
        media_index_field,
        media_constants.TEST_ID_PREFIX,
        media_type
      );
      return pdf_storage;
    },
    ...
    PodcastMediaCreate: function () {
      ...
      var podcast_storage = new PodcastMedia(
        media_index_field,
        media_constants.TEST_ID_PREFIX,
        media_type
      );
    ...
    VersionStorageCreate: function (current_media, media_description_text) {
      // PdfMedia or RsdMedia
      var media_methods = [
        "getTsvVariables",
        "deleteItems",
        "databaseItem",
      ...
      var missing_methods = miscMethods.objectHasMethods(
        current_media,
        media_methods
      );
      //var  missing_methods = 'exception test'
      if (missing_methods) {
        throw new Error(
          "VersionStorageCreate.missing_methods() " + missing_methods
        );
      } else {
        var version_storage = new VersionStorage(
          current_media,
