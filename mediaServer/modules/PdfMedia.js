...
PdfMedia.prototype.rssFeed = function (
  data_rows,
...
PdfMedia.prototype.getDocument = function (
  document_id,
  field_id,
  real_or_test
) {
...
PdfMedia.prototype.deleteDocument = function (
  document_name,
  real_or_test /* :string */
) {
...
PdfMedia.prototype.upsertDocument = function (
  document_name,
  variable_name,
  the_value,
  real_or_test
...
PdfMedia.prototype.splitVersions = function (
  data_rows_compact,
  offset_minutes,
  tsv_variables
) {
  ...
  for (const row_index in data_rows_compact) {
    ...
    for (var i = 1; i <= media_constants.PDF_MAX_LINKS; i++) {
      if (cleaned_row['pdf link ' + i]) {
        var prototype_data_row = objectAssign({}, cleaned_row);
        prototype_data_row['file name'] = cleaned_row['pdf link ' + i];
        prototype_data_row['page_count'] =
          prototype_data_row['pdf page count ' + i];
        prototype_data_row['country_location'] =
          prototype_data_row['pdf country ' + i];
        prototype_data_row['publisher_info'] =
          prototype_data_row['pdf info ' + i];
        let alphabet_offset = String.fromCharCode(64 + i);
        prototype_data_row['episode number'] =
          rss_record_start_y + prototype_data_row['episode number'] * 10 + i;
        prototype_data_row['version_offset'] = alphabet_offset;
        let single_pdf = this.deleteAllVersions(prototype_data_row);
        let modified_row = this.deriveData(
          single_pdf,
          offset_minutes,
          tsv_variables
        );
...
PdfMedia.prototype.deriveData = function (
  data_columns,
  offset_minutes,
  tsv_variables
) {
...
PdfMedia.prototype._pageTemplateVars = function (
  rss_items_html,
  tsv_variable_information,
  itunes_summary
) {
